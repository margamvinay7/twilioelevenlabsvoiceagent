...
const elevenlabs = new ElevenLabsClient({
  apiKey: process.env.ELEVENLABS_API_KEY, // must be defined
});
const app = express();
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", async (raw) => {
    ...
    if (evt.event === "media") {
      ...
      if (callSession.buffer.length >= 16000 && !callSession.isProcessing) {
...
async function generateSalesResponse(userText, callSession) {
  try {
    ...
    const salesContext = `
You are a professional telecom sales agent speaking directly with a customer over a call. 
Your job is to convince them to recharge their SIM card by highlighting bonuses, discounts, and special offers. 
Always sound friendly, persuasive, and natural like a real human agent—not like an AI. 
Keep responses short (1–2 sentences), conversational, and relevant to what the customer just said. 
Do not give robotic or generic answers—speak as if you are actually on the call.

Conversation so far:
${callSession.conversationHistory.map(msg => `${msg.role}: ${msg.content}`).join('\n')}

Customer just said: "${userText}"

Now respond naturally in English with a short reply that feels like a real telecom agent talking on a live call.
`;
...
async function speakText(ws, text) {
  ...
  // Insert a pause marker so Twilio waits before resuming mic
  ws.send(JSON.stringify({ event: "mark", mark: { name: "end_of_ai" } }));
}
